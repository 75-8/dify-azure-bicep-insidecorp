...
- Bicep モジュールの境界定義
- 各モジュール間の依存関係
- [main.bicep] におけるデプロイ順序

## モジュール対応表 (実装状況)

| 役割名 | 物理 Bicep テンプレートファイル | 概要 |
|---|---|---|
| `nsgs` | [nsg.bicep] | ネットワークセキュリティグループの作成 (AppGw, ACA, PrivateLink, Postgres) |
| `network` | [network.bicep] | 仮想ネットワーク (VNet) および各サブネット (委譲設定・NSGバインド含む) の作成 |
| `keyvault` | [keyvault.bicep] | Key Vault、Private Endpoint およびプライベート DNS 設定の作成 |
...
[main.bicep] におけるモジュールの作成順序（依存性）は以下の順で行われる：
