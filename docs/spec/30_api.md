...
- **制限**: 現行モジュールは APIM インスタンスの作成のみを行う最小限の定義であり、[main.bicep] からはまだ呼び出されていない。また、OpenAPI 定義のインポート設定や、OAuth2 のトークン検証ポリシー定義は含まれていない。

## 将来の課題（APIM 統合時のロードマップ）
- **[main.bicep] への組み込み**: APIM モジュール呼び出しの有効化。
